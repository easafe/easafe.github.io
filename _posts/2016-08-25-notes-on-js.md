...
Functions as first class citizens are really nice, however JavaScript, as it does with everything, takes it a little extreme. The only thing which might be as versatile as a JavaScript function is [Hokuto Shinken](https://hokuto.fandom.com/wiki/Hokuto_Shinken) - but that can only be used by a single person at any given time whereas JavaScript is used by millions of unsuspicious programmers every day.
...
Then sprinkled in somewhere in the middle to complete the seamlessness, functions also double as "classes"
...
JavaScript has no concept of block scoping. Instead, functions (again) determine the scope of variables, which forces you to write code as follows
...
just in case you feel like reusing a variable name. Misspelled an identifier? Don't worry, the JavaScript engine will kindly infer you mean something declared elsewhere. Need to hide some implementation details? Have fun with
...
JavaScript brings to the table all the worst of dynamic typing. Like a duck that quacks like a duck, swims like a duck, walks like a duck but is in fact a platypus. There are no warnings or errors for assigning undeclared variables. Objects, even core ones, can be extended willy nilly. You can compare distinct types. God may have mercy on your soul if you try to reason about comparision behavior. Or typeof. Or casting.

What do you think it is gonna be [commited](https://gist.github.com/glauberramos/3087548) at 3am when caffeine has run out?
